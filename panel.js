...
app.get('/explore/:bot',(req,res)=>{
  ...
  const entries=fs.readdirSync(dir).map(name=>{ const full=path.join(dir,name); return {name,isDir:fs.statSync(full).isDirectory()}; });
...
io.on('connection',socket=>{
  ...
  socket.on('action', data=>{
    ...
    } else if(data.cmd==='run-html'){
      if(proc) proc.kill();
      proc=spawn('npx',['http-server','.','-p',data.port],{cwd:path.join(BOTS_DIR,data.bot)});
...
http.listen(3000,()=>console.log('ADPanel_Final_v15 on http://localhost:3000'));
