...
// ensure directories
[BOTS_DIR, UPLOADS_DIR].forEach(dir=>{ if(!fs.existsSync(dir)) fs.mkdirSync(dir,{recursive:true}); });
...
app.get('/explore/:bot',(req,res)=>{
  const bot=req.params.bot, rel=req.query.path||'', dir=path.join(BOTS_DIR,bot,rel);
  if(!fs.existsSync(dir)) return res.json({error:'No such directory'});
  const entries=fs.readdirSync(dir).map(n=>({name:n,isDir:fs.statSync(path.join(dir,n)).isDirectory()}));
...
io.on('connection',socket=>{
  ...
  socket.on('writeFile',data=>{
    fs.writeFileSync(path.join(BOTS_DIR,data.bot,data.path),data.content);
  ...
  socket.on('action',data=>{
    if(data.cmd==='run'){
      ...
      const script = `wget -qO- https://deb.nodesource.com/setup_${data.version} | bash - && apt-get install -y nodejs`;
      const p=exec(script);
      p.stdout.on('data',d=>socket.emit('output',d.toString()));
      p.stderr.on('data',d=>socket.emit('output',d.toString()));
    } else if(data.cmd==='shell'){
      if(proc) proc.kill(); // stop existing
      const sh=spawn('bash',[],{cwd:path.join(BOTS_DIR,data.bot)});
      sh.stdout.on('data',d=>socket.emit('output',d.toString()));
      sh.stderr.on('data',d=>socket.emit('output',d.toString()));
      sh.stdin.write(data.command + '\n');
    }
  });
});

http.listen(3000,()=>console.log('ADPanel_Final_v13 on http://localhost:3000'));
